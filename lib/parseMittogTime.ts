...
/**
 * Parse a time string and produce a Unix timestamp (ms).
 *
 * Forms accepted:
 * - Full date: "DD-MM-YYYY HH:mm" (optional :ss)
 * - Time-only: "HH:mm" (optional :ss) interpreted as today or next day
 * - Any ISO/standard date string (fallback)
 *
 * Options:
 * - referenceNow: number (ms since epoch) used as the "now" reference for deterministic parsing
 * - thresholdMs: number override for the rollover heuristic (default 12 hours)
 */
export function parseMittogTime(
    time: string,
    options?: { referenceNow?: number; thresholdMs?: number }
) {
